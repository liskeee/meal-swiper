...
export default function CookingPage() {
  ...
  // Effective day: use explicit selection if it has a meal, otherwise fall back to default
  const effectiveDay = selectedDay && weeklyPlan[selectedDay] ? selectedDay : defaultDay

  const meal: Meal | null = effectiveDay ? weeklyPlan[effectiveDay] : null

  const [checkedSteps, setCheckedSteps] = useState<Record<number, boolean>>({})
  const [checkedIngredients, setCheckedIngredients] = useState<Record<string, boolean>>({})

  // Reset checkboxes when day changes
